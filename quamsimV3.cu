...
__global__ void
mat_mul(float *d_u, float *d_ip,float *d_op,int q,int size)
{
	...
	j=j+segment2*q;
	 ...
	 s[i_four+3]=d_ip[j+q]; 
	  ...
	  d_op[j]=(d_u[0]*s[i_four+2]) + (d_u[1]*s[i_four+3]);                 
	  d_op[j+q]=(d_u[2]*s[i_four+2])+(d_u[3]*s[i_four+3])  ;  
	
	 d_op[i]=(d_u[0]*s[i_four]) + (d_u[1]*s[i_four+1]);                
	  d_op[i+q]=(d_u[2]*s[i_four])+(d_u[3]*s[i_four+1])  ; 
