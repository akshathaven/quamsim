...
__global__ void mat_mul(float *d_u, float *d_ip,float *d_op,int qubit,int size)
{
	  __shared__ float s[64];

	int i = blockDim.x * blockIdx.x + threadIdx.x;
	int segment1 = i/qubit;
	//int i_four=2*threadIdx.x;
int j=i+(size/4);
int segment2=j/qubit;
...
j=j+segment2*qubit;
	...
	i=i+segment1*qubit;
	 s[4*threadIdx.x]=d_ip[i]; 
	  ...
	  s[4*threadIdx.x+3]=d_ip[j+qubit];
	
	 
   	  __syncthreads();

	  d_op[i]=(d_u[0]*s[4*threadIdx.x+2]) + (d_u[1]*s[4*threadIdx.x+3]);                 
	  d_op[i+qubit]=(d_u[2]*s[4*threadIdx.x+2])+(d_u[3]*s[4*threadIdx.x+3])  ;

	d_op[i]=(d_u[0]*s[4*threadIdx.x]) + (d_u[1]*s[4*threadIdx.x+1]);                 
	  d_op[i+qubit]=(d_u[2]*s[4*threadIdx.x])+(d_u[3]*s[4*threadIdx.x+1])  ; 
   ...
   // //qubit_circuit = argv[0]; //qubit circuit
	...
	cudaMemcpy(d_u,u1,4*sizeof(float),cudaMemcpyHostToDevice);
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[0]),test_size);
	cudaMemcpy(ip,d_op,(count-6)*sizeof(float),cudaMemcpyDeviceToHost);
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[1]),test_size);
	cudaMemcpy(ip,d_op,(count-6)*sizeof(float),cudaMemcpyDeviceToHost);
	...
	//3rd
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[2]),test_size);
	cudaMemcpy(ip,d_op,(count-6)*sizeof(float),cudaMemcpyDeviceToHost);
	...
	cudaMemcpy(d_u,u4,4*sizeof(float),cudaMemcpyHostToDevice);
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[3]),test_size);
	cudaMemcpy(ip,d_op,(count-6)*sizeof(float),cudaMemcpyDeviceToHost);
	...
	cudaMemcpy(d_u,u5,4*sizeof(float),cudaMemcpyHostToDevice);
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[4]),test_size);
	cudaMemcpy(ip,d_op,(count-6)*sizeof(float),cudaMemcpyDeviceToHost);
	...
	cudaMemcpy(d_u,u6,4*sizeof(float),cudaMemcpyHostToDevice);
	 ...
	 cudaMemcpy(d_op,op,(count-6)*sizeof(float),cudaMemcpyHostToDevice);
		...
		mat_mul<<<num_frag, th_block>>>(d_u,d_ip,d_op, pow(2,qubit[5]),test_size);
